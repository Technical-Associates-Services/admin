    ...
    <DashboardLayout>
      <div className="flex flex-col h-full space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Shops / Branches</h1>
            <p className="text-muted-foreground">Manage your offline stores or branches.</p>
          </div>
        ...
        <Card className="flex-1 border-zinc-200/60 dark:border-zinc-800/60 shadow-sm overflow-hidden flex flex-col">
          ...
          <CardContent className="p-0 flex-1 overflow-auto">
            {isLoading ? (
              ...
              <div className="flex h-full items-center justify-center text-muted-foreground py-12">No shops found. Add one!</div>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {items.map((item) => (
                    <TableRow key={item.id}>
                      <TableCell>
                        {item.image ? (
                          <div className="w-12 h-12 overflow-hidden rounded">
                            <img 
                              src={item.image ? (item.image.startsWith('http') ? item.image : `${process.env.NEXT_PUBLIC_API_URL}/frontend/images/shops/${item.image}`) : '/logo.png'} 
                              alt={item.title} 
                              className="object-cover w-full h-full" 
                              onError={(e) => {
                                if (!e.currentTarget.dataset.retried && item.image && !item.image.startsWith('http')) {
                                  e.currentTarget.dataset.retried = "true";
                                  e.currentTarget.src = `https://ivgvdteklcqtzfusbsfh.supabase.co/storage/v1/object/public/tas_media/shops/${item.image}`;
                                } else {
                      ...
                      <TableCell className="hidden md:table-cell text-muted-foreground">{item.website || 'N/A'}</TableCell>
