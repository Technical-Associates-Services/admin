...
import { ConcernForm } from '@/components/forms/ConcernForm';
    ...
    <DashboardLayout>
      <div className="flex flex-col h-full space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Sister Concerns</h1>
            <p className="text-muted-foreground">Manage your sister concern companies.</p>
          </div>
        ...
        <Card className="flex-1 border-zinc-200/60 dark:border-zinc-800/60 shadow-sm overflow-hidden flex flex-col">
          <CardHeader className="py-4 px-6 border-b border-zinc-100 dark:border-zinc-800/60 bg-zinc-50/50 dark:bg-zinc-900/20">
            <CardTitle className="text-lg">All Sister Concerns</CardTitle>
          </CardHeader>
          <CardContent className="p-0 flex-1 overflow-auto">
            {isLoading ? (
              <Loader />
            ) : items.length === 0 ? (
              <div className="flex h-full items-center justify-center text-muted-foreground py-12">No items found. Add one!</div>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
                  ...
                  {items.map((item) => (
                    <TableRow key={item.id}>
                      <TableCell>
                        {item.image ? (
                          <img 
                            src={item.image ? (item.image.startsWith('http') ? item.image : `${process.env.NEXT_PUBLIC_API_URL}/frontend/images/concerns/${item.image}`) : '/logo.png'} 
                            alt={item.title} 
                            className="object-cover w-12 h-12 rounded" 
                            onError={(e) => {
                              if (!e.currentTarget.dataset.retried && item.image && !item.image.startsWith('http')) {
                                e.currentTarget.dataset.retried = "true";
                                e.currentTarget.src = `https://ivgvdteklcqtzfusbsfh.supabase.co/storage/v1/object/public/tas_media/concerns/${item.image}`;
                              } else {
                      ...
                      <TableCell className="hidden md:table-cell text-muted-foreground">{item.links || 'N/A'}</TableCell>
