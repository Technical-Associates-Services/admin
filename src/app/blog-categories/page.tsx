...
export default function BlogCategoriesPage() {
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Are you sure you want to delete this category? Make sure no blogs are attached to it.')) {
      try {
        await api.delete(`/admin/blog-categories/${id}`);
        fetchItems();
      } catch (error) { alert('Failed to delete. Ensure no blogs use this category first.'); }
    }
  };
      ...
      <div className="flex flex-col h-full space-y-4">
        ...
        <Card className="flex-1 border-zinc-200/60 dark:border-zinc-800/60 shadow-sm overflow-hidden flex flex-col">
          <CardHeader className="py-4 px-6 border-b border-zinc-100 dark:border-zinc-800/60 bg-zinc-50/50 dark:bg-zinc-900/20">
            <CardTitle className="text-lg">All Blog Categories</CardTitle>
          </CardHeader>
          <CardContent className="p-0 flex-1 overflow-auto">
            {isLoading ? (
              <Loader />
            ) : items.length === 0 ? (
              <div className="flex h-full items-center justify-center text-muted-foreground py-12">No categories found. Add one!</div>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
                      ...
                      <TableCell>
                        {item.image ? (
                          <div className="w-12 h-12 overflow-hidden rounded">
                            <img 
                              src={item.image ? (item.image.startsWith('http') ? item.image : `${process.env.NEXT_PUBLIC_API_URL}/frontend/images/blog_categories/${item.image}`) : '/logo.png'} 
                              alt={item.title} 
                              className="object-cover w-full h-full" 
                              onError={(e) => {
                                if (!e.currentTarget.dataset.retried && item.image && !item.image.startsWith('http')) {
                                  e.currentTarget.dataset.retried = "true";
                                  e.currentTarget.src = `https://ivgvdteklcqtzfusbsfh.supabase.co/storage/v1/object/public/tas_media/blog_categories/${item.image}`;
                                } else {
