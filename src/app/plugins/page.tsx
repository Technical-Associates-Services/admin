...
export default function PluginsPage() {
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Are you sure you want to delete this plugin code?')) {
      try {
    ...
    <DashboardLayout>
      <div className="flex flex-col h-full space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Custom Plugins & Scripts</h1>
            <p className="text-muted-foreground">Manage your tracking pixels and custom HTML/JS code injections.</p>
          </div>
        ...
        <Card className="flex-1 border-zinc-200/60 dark:border-zinc-800/60 shadow-sm overflow-hidden flex flex-col">
          ...
          <CardContent className="p-0 flex-1 overflow-auto">
            {isLoading ? (
              ...
              <div className="flex h-full items-center justify-center text-muted-foreground py-12">No plugins found. Add one!</div>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="hidden md:table-cell">Placement (Tag Type)</TableHead>
                ...
                <TableBody>
                  {items.map((item) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell className="hidden md:table-cell text-muted-foreground capitalize">{item.type}</TableCell>
                      <TableCell className="hidden md:table-cell text-muted-foreground capitalize">
                         <span className="inline-block px-2 py-1 bg-zinc-100 dark:bg-zinc-800 rounded font-mono text-xs text-blue-500">
                           &lt;{item.tag_type}&gt;
                         </span>
                      </TableCell>
