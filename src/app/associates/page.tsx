      ...
      <div className="flex flex-col h-full space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Manage your associate partners.</p>
          </div>
          <Button onClick={() => { setSelected(null); setIsFormOpen(true); }} className="gap-2">
            <Plus className="h-4 w-4" /> Add Associate
          </Button>
        </div>
        ...
        <Card className="flex-1 border-zinc-200/60 dark:border-zinc-800/60 shadow-sm overflow-hidden flex flex-col">
          <CardHeader className="py-4 px-6 border-b border-zinc-100 dark:border-zinc-800/60 bg-zinc-50/50 dark:bg-zinc-900/20">
            <CardTitle className="text-lg">All Associates</CardTitle>
          </CardHeader>
          <CardContent className="p-0 flex-1 overflow-auto">
            {isLoading ? (
              <Loader />
            ) : items.length === 0 ? (
              <div className="flex h-full items-center justify-center text-muted-foreground py-12">No associates found. Add one!</div>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
                  ...
                  {items.map((item) => (
                    <TableRow key={item.id}>
                      <TableCell>
                        {item.image ? (
                          <img 
                            src={item.image ? (item.image.startsWith('http') ? item.image : `${process.env.NEXT_PUBLIC_API_URL}/frontend/images/associates/${item.image}`) : '/logo.png'} 
                            alt={item.title} 
                            className="object-cover w-12 h-12 rounded" 
                            onError={(e) => {
                              if (!(e.currentTarget as any).dataset.retried && item.image && !item.image.startsWith('http')) {
                                (e.currentTarget as any).dataset.retried = "true";
                                e.currentTarget.src = `https://ivgvdteklcqtzfusbsfh.supabase.co/storage/v1/object/public/tas_media/associations/${item.image}`;
                              } else {
                      ...
                      <TableCell className="hidden md:table-cell text-muted-foreground">{item.category?.title || 'General'}</TableCell>
