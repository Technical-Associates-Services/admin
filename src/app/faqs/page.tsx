        ...
        <Card className="flex-1 border-zinc-200/60 dark:border-zinc-800/60 shadow-sm overflow-hidden flex flex-col">
          ...
          <CardContent className="p-0 flex-1 overflow-auto">
            {isLoading ? (
              ...
              <div className="flex h-full items-center justify-center text-muted-foreground py-12">No FAQs found. Add one!</div>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {items.map((item) => (
                    <TableRow key={item.id}>
                      <TableCell className="font-medium max-w-[400px] truncate">{item.question}</TableCell>
                      <TableCell className="hidden md:table-cell text-muted-foreground">{item.name || 'Admin'}</TableCell>
