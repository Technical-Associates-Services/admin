      ...
      <div className="flex flex-col h-full space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Manage your corporate services.</p>
          </div>
        ...
        <Card className="flex-1 border-zinc-200/60 dark:border-zinc-800/60 shadow-sm overflow-hidden flex flex-col">
          ...
          <CardContent className="p-0 flex-1 overflow-auto">
            {isLoading ? (
              ...
              <div className="flex h-full items-center justify-center text-muted-foreground py-12">No services found. Add one!</div>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
                  ...
                  {items.map((item) => (
                    <TableRow key={item.id}>
                      <TableCell>
                        {item.image ? (
                          <img 
                            src={item.image ? (item.image.startsWith('http') ? item.image : `${process.env.NEXT_PUBLIC_API_URL}/frontend/images/services/${item.image}`) : '/logo.png'} 
                            alt={item.title} 
                            className="w-12 h-12 object-cover rounded" 
                            onError={(e) => {
                              if (!e.currentTarget.dataset.retried && item.image && !item.image.startsWith('http')) {
                                e.currentTarget.dataset.retried = "true";
                                e.currentTarget.src = `https://ivgvdteklcqtzfusbsfh.supabase.co/storage/v1/object/public/tas_media/services/${item.image}`;
                              } else {
                      ...
                      <TableCell className="hidden md:table-cell text-muted-foreground max-w-[300px] truncate">{item.summary}</TableCell>
